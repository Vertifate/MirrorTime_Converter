...
def render_preprocess(cluster_origin:torch.Tensor|None,cluster_extend:torch.Tensor|None,frustumplane:torch.Tensor,view_matrix:torch.Tensor,
    ...
    if pp.cluster_size:
        ...
        if pp.sparse_grad:#enable sparse gradient
            result = utils.wrapper.CullCompactActivateWithSparseGrad.apply(
            ...
            if result is None:
                ...
                raise RuntimeError("CullCompactActivateWithSparseGrad returned None")
            if len(result) < 6:
                # Handle the case where the function returns incomplete results
                raise RuntimeError(f"CullCompactActivateWithSparseGrad returned {len(result)} values, expected at least 6")
            visible_chunkid,culled_xyz,culled_scale,culled_rot,color,culled_opacity = result
