...
#ifndef __hgt2_mask
// Fallbacks for CUDA versions that don't provide half2 mask intrinsics
__device__ __forceinline__ unsigned int __hgt2_mask(half2 a, half2 b) {
    unsigned int mask = 0;
    mask |= (__half2float(__low2half(a)) > __half2float(__low2half(b))) ? 0x0000ffffu : 0;
    mask |= (__half2float(__high2half(a)) > __half2float(__high2half(b))) ? 0xffff0000u : 0;
    return mask;
}
#endif

#ifndef __hge2_mask
__device__ __forceinline__ unsigned int __hge2_mask(half2 a, half2 b) {
    unsigned int mask = 0;
    mask |= (__half2float(__low2half(a)) >= __half2float(__low2half(b))) ? 0x0000ffffu : 0;
    mask |= (__half2float(__high2half(a)) >= __half2float(__high2half(b))) ? 0xffff0000u : 0;
    return mask;
